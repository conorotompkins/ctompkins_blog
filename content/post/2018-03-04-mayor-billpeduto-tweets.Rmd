...
df_bill %>% 
  ...
  scale_color_viridis(name = "Weekday", discrete = TRUE) +
  labs(title = title,
       x = "",
...
The Mayor does most of his tweeting around lunchtime, though he is known to tweet in the late evening as well.
```{r hour}
df_bill %>% 
  count(hour) %>% 
...
This view combines the weekday and hour variables, and confirms what we observed when looking at the variables independently.
```{r heatmap}
df_bill %>% 
...
This plot shows that over half of the Mayor's tweets are retweets, regular tweets make up 20-30% of the tweets, quote tweets make up about 10-20%, and replies supply the remainder.
```{r tweet_types}
df_bill %>% 
...
By separating the Mayor's tweets into bigrams (two-word chunks), we can identify which words are used together often. This code cuts the tweets into bigrams and counts their frequency.
...
This network graph shows which words the Mayor uses together frequently. "public" + "safety", and "affordable" + "housing" are the main standouts. I think this plot pretty accurately captures the policy issues the Mayor focuses on.
...
This plot shows which words are correlated highly with each other. Some words are "stemmed" (shortened/modified), to capture very similar words, like "business" and "businesses". "amazon" and "hq2" are used together overwhelmingly. The "pwsa" + "water" + "decad" combination indicate the ongoing water quality problems, and plans for reinvestment in the infrastructure.
