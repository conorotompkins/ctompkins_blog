...
#cut hour column
simple_network <- data_wide %>% 
  mutate(time_of_day = cut(hour, c(0, 6, 12, 18, 24), labels = c("early morning", "Mid day", "Evening", "Night time"), ordered_result = TRUE)) %>% 
  count(is_weekday, time_of_day, from_station_name, to_station_name, sort = TRUE) %>% 
  top_n(400)

str(simple_network$time_of_day)
