...
[Set your working directory](http://rprogramming.net/set-working-directory-in-r/)

Then, read the data into R
```{r read data into r}
df <- read_csv("https://raw.githubusercontent.com/conorotompkins/pgh-crime/master/data/archive-police-blotter.csv")
```
...
Before we start making plots, we will set a theme
```{r load theme}
theme_set(theme_bw())
```

How has the number of incidents changed over time?
```{r incidents over time, message=FALSE, warning=FALSE}
df %>% 
...
Bob Gradeck pointed out that [Zone 6 was closed in 2003 and reopened in 2008](http://www.post-gazette.com/local/neighborhoods/2008/03/31/Police-manpower-tipped-to-West-End-s-Zone-6/stories/200803310145)
...
```{r create map, message=FALSE, warning=FALSE, cache=TRUE}
